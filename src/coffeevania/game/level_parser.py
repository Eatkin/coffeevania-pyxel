...
) -> Dict[str, Optional[Callable[[int, int], CoffeevaniaEntity]]]:
    return {
        ...
        "B": lambda x, y: context.app.create_entity(
            BouncingHazard, position=Position(x, y)
        ),
...
def load_level(name: str, offset_x: int, offset_y: int, context: GlobalContext) -> None:
...
def load_world(context: GlobalContext) -> None:
    ...
    for row in layout:
        ...
        for chunk in row:
            if chunk:
                load_level(chunk, x_offset, y_offset, context)
