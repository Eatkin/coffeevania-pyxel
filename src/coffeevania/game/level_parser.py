...
def get_legend(
    ...
    return {
        ...
        "c": lambda x, y: context.app.create_entity(
            Checkpoint, position=Position(x, y)
        ),
...
def load_level(
    ...
    conveyor_queue: List[Tuple[Position, int]],
) -> None:
        ...
        for y, line in enumerate(f):
            ...
            for x, char in enumerate(stripped):
                if char in legend and legend[char]:
                    if char == "#":
                        block_queue.append(
                            Position(x * GRID_SIZE + offset_x, y * GRID_SIZE + offset_y)
                        )
                    elif char == "B" or char == "b":
                        # Jank ass shit for conveyor belts
                        conveyor_queue.append(
                            (
                                Position(
...
def load_world(context: GlobalContext) -> CollisionMapType:
    ...
    for row in layout:
        ...
        for chunk in row:
            if chunk:
                load_level(
                    chunk, x_offset, y_offset, context, block_queue, conveyor_queue
                )
            x_offset += chunk_width * GRID_SIZE
        y_offset += chunk_height * GRID_SIZE

    collision_map: CollisionMapType = {}

    # Create all the conveyor belts and blocks
    legend = get_legend(context)
    for p, d in conveyor_queue:
        legend["B"](p.x, p.y)  # type: ignore
        # Add to collision map
        x_cell, y_cell = p.x // GRID_SIZE, p.y // GRID_SIZE
        collision_map[(int(x_cell), int(y_cell))] = d
